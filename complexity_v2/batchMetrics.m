...
%            samp_en_call(handle, handle.brainMask, handles.metrics_master{j}.m, handles.metrics_master{j}.r);
       elseif strcmp(handles.metrics_master{j}.metric,'fuzzy')
           fuzzy_en_call(handle, handle.brainMask, handles.metrics_master{j}.m, handles.metrics_master{j}.r, handles.metrics_master{j}.n, handles.metrics_master{j}.tau);
       elseif strcmp(handles.metrics_master{j}.metric,'mse')
           mse_en_call(handle, handle.brainMask, handles.metrics_master{j}.m, handles.metrics_master{j}.r, handles.metrics_master{j}.a);
       elseif strcmp(handles.metrics_master{j}.metric,'perm')
           perm_en_call(handle, handle.brainMask, handles.metrics_master{j}.o, handles.metrics_master{j}.tau, handles.metrics_master{j}.norm);
       end
    end
end
 
function result = loadMetrics(hObject, handles)
index = 1;
...
if handles.mse
    if ~(isfield(handles,'mse_rMin'))
        handles.mse_rMin=0;
    end
    if ~(isfield(handles,'mse_rMax'))
        handles.mse_rMax=0;
    end
    if ~(isfield(handles,'mse_rStep'))
        handles.mse_rStep=0;
    end
    if ~(isfield(handles,'mse_mMin'))
        handles.mse_mMin=0;
    end
    if ~(isfield(handles,'mse_mMax'))
        handles.mse_mMax=0;
    end
    if ~(isfield(handles,'mse_mStep'))
        handles.mse_mStep=0;
    end
    if ~(isfield(handles,'mse_a'))
        handles.mse_a=1;
    end
    rList = metricListFunc(handles.mse_rMin, handles.mse_rMax, handles.mse_rStep);
    mList = metricListFunc(handles.mse_mMin, handles.mse_mMax, handles.mse_mStep);
    aList = metricListFunc(1,handles.mse_a,1);
    disp(aList)
    for idx = 1:length(rList) 
        for jdx = 1:length(mList)
            for adx = 1:length(aList)
                metrics_master{index} = struct('metric','mse','r',rList(idx), 'm', mList(jdx),'a',aList(adx));
                index = index+1;
            end
        end 
    end
...
if handles.perm
    if ~(isfield(handles,'perm_oMin'))
        handles.perm_oMin=0;
    end
    if ~(isfield(handles,'perm_oMax'))
        handles.perm_oMax=0;
    end
    if ~(isfield(handles,'perm_oStep'))
        handles.perm_oStep=0;
    end
    if ~(isfield(handles,'perm_tauMin'))
        handles.perm_tauMin=0;
    end
    if ~(isfield(handles,'perm_tauMax'))
        handles.perm_tauMax=0;
    end
    if ~(isfield(handles,'perm_tauStep'))
        handles.perm_tauStep=0;
    end
    if ~(isfield(handles,'perm_norm'))
        handles.perm_norm=0;
    end
    
    oList = metricListFunc(handles.perm_oMin, handles.perm_oMax, handles.perm_oStep);
...
function perm_en_call(handles, mask, order, delay, normalize)
imgSize = size(mask);
brainVox = find(mask == max(mask(:)));
...
% --- Executes on button press in checkboxPerm.
function checkboxPerm_Callback(hObject, eventdata, handles)
% Hint: get(hObject,'Value') returns toggle state of checkboxPerm
value = get(hObject,'Value');
handles.perm = value;
guidata(hObject, handles);


function oStepPerm_Callback(hObject, eventdata, handles)
% Hints: get(hObject,'String') returns contents of oStepPerm as text
%        str2double(get(hObject,'String')) returns contents of oStepPerm as a double
oStep = get(hObject,'String');
oStep = str2num(oStep);
handles.perm_oStep = oStep;
guidata(hObject, handles);

% --- Executes during object creation, after setting all properties.
function oStepPerm_CreateFcn(hObject, eventdata, handles)
% Hint: edit controls usually have a white background on Windows.
%       See ISPC and COMPUTER.
if ispc && isequal(get(hObject,'BackgroundColor'), get(0,'defaultUicontrolBackgroundColor'))
    set(hObject,'BackgroundColor','white');
...
function tauStepPerm_Callback(hObject, eventdata, handles)
% Hints: get(hObject,'String') returns contents of tauStepPerm as text
%        str2double(get(hObject,'String')) returns contents of tauStepPerm as a double
tauStep = get(hObject,'String');
tauStep = str2num(tauStep);
handles.perm_tauStep = tauStep;
guidata(hObject, handles);

% --- Executes during object creation, after setting all properties.
function tauStepPerm_CreateFcn(hObject, eventdata, handles)
% Hint: edit controls usually have a white background on Windows.
%       See ISPC and COMPUTER.
if ispc && isequal(get(hObject,'BackgroundColor'), get(0,'defaultUicontrolBackgroundColor'))
    set(hObject,'BackgroundColor','white');
...
function tauMinPerm_Callback(hObject, eventdata, handles)
% Hints: get(hObject,'String') returns contents of tauMinPerm as text
%        str2double(get(hObject,'String')) returns contents of tauMinPerm as a double
tauMin = get(hObject,'String');
tauMin = str2num(tauMin);
handles.perm_tauMin = tauMin;
guidata(hObject, handles);

% --- Executes during object creation, after setting all properties.
function tauMinPerm_CreateFcn(hObject, eventdata, handles)
% Hint: edit controls usually have a white background on Windows.
%       See ISPC and COMPUTER.
if ispc && isequal(get(hObject,'BackgroundColor'), get(0,'defaultUicontrolBackgroundColor'))
    set(hObject,'BackgroundColor','white');
...
function tauMaxPerm_Callback(hObject, eventdata, handles)
% Hints: get(hObject,'String') returns contents of tauMaxPerm as text
%        str2double(get(hObject,'String')) returns contents of tauMaxPerm as a double
tauMax = get(hObject,'String');
tauMax = str2num(tauMax);
handles.perm_tauMax = tauMax;
guidata(hObject, handles);

% --- Executes during object creation, after setting all properties.
function tauMaxPerm_CreateFcn(hObject, eventdata, handles)
% Hint: edit controls usually have a white background on Windows.
%       See ISPC and COMPUTER.
if ispc && isequal(get(hObject,'BackgroundColor'), get(0,'defaultUicontrolBackgroundColor'))
    set(hObject,'BackgroundColor','white');
...
function oMinPerm_Callback(hObject, eventdata, handles)
% Hints: get(hObject,'String') returns contents of oMinPerm as text
%        str2double(get(hObject,'String')) returns contents of oMinPerm as a double
oMin = get(hObject,'String');
oMin = str2num(oMin);
handles.perm_oMin = oMin;
guidata(hObject, handles);

% --- Executes during object creation, after setting all properties.
function oMinPerm_CreateFcn(hObject, eventdata, handles)
% Hint: edit controls usually have a white background on Windows.
%       See ISPC and COMPUTER.
if ispc && isequal(get(hObject,'BackgroundColor'), get(0,'defaultUicontrolBackgroundColor'))
    set(hObject,'BackgroundColor','white');
...
function oMaxPerm_Callback(hObject, eventdata, handles)
% hObject    handle to oMaxPerm (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of oMaxPerm as text
%        str2double(get(hObject,'String')) returns contents of oMaxPerm as a double
oMax = get(hObject,'String');
oMax = str2num(oMax);
handles.perm_oMax = oMax;
guidata(hObject, handles);

% --- Executes during object creation, after setting all properties.
function oMaxPerm_CreateFcn(hObject, eventdata, handles)
% Hint: edit controls usually have a white background on Windows.
%       See ISPC and COMPUTER.
if ispc && isequal(get(hObject,'BackgroundColor'), get(0,'defaultUicontrolBackgroundColor'))
    set(hObject,'BackgroundColor','white');
...
% --- Executes on button press in checkboxNormPerm.
function checkboxNormPerm_Callback(hObject, eventdata, handles)
% Hint: get(hObject,'Value') returns toggle state of checkboxNormPerm
value = get(hObject,'Value');
handles.perm_norm = value;
guidata(hObject, handles);
...
%*************************************************************************
%**********************MSE EN START ***************************************
function mse_en_call(handles, mask, m, r, a)
imgSize = size(mask);
brainVox = find(mask == max(mask(:)));
MseEn = zeros(imgSize);
...
niiStruct = make_nii(MseEn, handles.imgVoxDim, [], 64, []);
niiStruct.hdr.hk.data_type = 'float64';
save_nii(niiStruct, opFname, []);

% --- Executes on button press in checkboxMSE.
function checkboxMSE_Callback(hObject, eventdata, handles)
% Hint: get(hObject,'Value') returns toggle state of checkboxMSE
value = get(hObject,'Value');
handles.mse = value;
guidata(hObject, handles);


function mMinMSE_Callback(hObject, eventdata, handles)
% Hints: get(hObject,'String') returns contents of mMinMSE as text
%        str2double(get(hObject,'String')) returns contents of mMinMSE as a double
mMin = get(hObject,'String');
mMin = str2num(mMin);
handles.mse_mMin = mMin;
guidata(hObject, handles);

% --- Executes during object creation, after setting all properties.
function mMinMSE_CreateFcn(hObject, eventdata, handles)
% Hint: edit controls usually have a white background on Windows.
%       See ISPC and COMPUTER.
if ispc && isequal(get(hObject,'BackgroundColor'), get(0,'defaultUicontrolBackgroundColor'))
    set(hObject,'BackgroundColor','white');
...
function mMaxMSE_Callback(hObject, eventdata, handles)
% Hints: get(hObject,'String') returns contents of mMaxMSE as text
%        str2double(get(hObject,'String')) returns contents of mMaxMSE as a double
mMax = get(hObject,'String');
mMax = str2num(mMax);
handles.mse_mMax = mMax;
guidata(hObject, handles);

% --- Executes during object creation, after setting all properties.
function mMaxMSE_CreateFcn(hObject, eventdata, handles)
if ispc && isequal(get(hObject,'BackgroundColor'), get(0,'defaultUicontrolBackgroundColor'))
    set(hObject,'BackgroundColor','white');
end

function rMinMSE_Callback(hObject, eventdata, handles)
% Hints: get(hObject,'String') returns contents of rMinMSE as text
%        str2double(get(hObject,'String')) returns contents of rMinMSE as a double
rMin = get(hObject,'String');
rMin = str2num(rMin);
handles.mse_rMin = rMin;
guidata(hObject, handles);

% --- Executes during object creation, after setting all properties.
function rMinMSE_CreateFcn(hObject, eventdata, handles)
% Hint: edit controls usually have a white background on Windows.
%       See ISPC and COMPUTER.
if ispc && isequal(get(hObject,'BackgroundColor'), get(0,'defaultUicontrolBackgroundColor'))
    set(hObject,'BackgroundColor','white');
...
function rStepMSE_Callback(hObject, eventdata, handles)
% Hints: get(hObject,'String') returns contents of rStepMSE as text
%        str2double(get(hObject,'String')) returns contents of rStepMSE as a double
rStep = get(hObject,'String');
rStep = str2num(rStep);
handles.mse_rStep = rStep;
guidata(hObject, handles);

% --- Executes during object creation, after setting all properties.
function rStepMSE_CreateFcn(hObject, eventdata, handles)
% Hint: edit controls usually have a white background on Windows.
%       See ISPC and COMPUTER.
if ispc && isequal(get(hObject,'BackgroundColor'), get(0,'defaultUicontrolBackgroundColor'))
    set(hObject,'BackgroundColor','white');
...
function rMaxMSE_Callback(hObject, eventdata, handles)
% Hints: get(hObject,'String') returns contents of rMaxMSE as text
%        str2double(get(hObject,'String')) returns contents of rMaxMSE as a double
rMax = get(hObject,'String');
rMax = str2num(rMax);
handles.mse_rMax = rMax;
guidata(hObject, handles);

% --- Executes during object creation, after setting all properties.
function rMaxMSE_CreateFcn(hObject, eventdata, handles)
% Hint: edit controls usually have a white background on Windows.
%       See ISPC and COMPUTER.
if ispc && isequal(get(hObject,'BackgroundColor'), get(0,'defaultUicontrolBackgroundColor'))
    set(hObject,'BackgroundColor','white');
...
function mStepMSE_Callback(hObject, eventdata, handles)
% Hints: get(hObject,'String') returns contents of mStepMSE as text
%        str2double(get(hObject,'String')) returns contents of mStepMSE as a double
mStep = get(hObject,'String');
mStep = str2num(mStep);
handles.mse_mStep = mStep;
guidata(hObject, handles);

% --- Executes during object creation, after setting all properties.
function mStepMSE_CreateFcn(hObject, eventdata, handles)
% Hint: edit controls usually have a white background on Windows.
%       See ISPC and COMPUTER.
if ispc && isequal(get(hObject,'BackgroundColor'), get(0,'defaultUicontrolBackgroundColor'))
    set(hObject,'BackgroundColor','white');
...
function a_mse_Callback(hObject, eventdata, handles)
% Hints: get(hObject,'String') returns contents of a_mse as text
%        str2double(get(hObject,'String')) returns contents of a_mse as a double
...
% --- Executes during object creation, after setting all properties.
function a_mse_CreateFcn(hObject, eventdata, handles)
% Hint: edit controls usually have a white background on Windows.
%       See ISPC and COMPUTER.
if ispc && isequal(get(hObject,'BackgroundColor'), get(0,'defaultUicontrolBackgroundColor'))
    set(hObject,'BackgroundColor','white');
