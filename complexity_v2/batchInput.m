...
% Last Modified by GUIDE v2.5 17-Nov-2021 16:53:01
...
is4D = get(handles.is4D,'Value');
is3D = get(handles.is3D, 'Value');
if (strcmp(handles.is3D4D,'')==1 || (is3D==0 && is4D==0))
    handles.is3D4D="";
    msgbox('Please choose the input format/dimension: 3D or 4D');
    guidata(hObject, handles);
    return
else
    dirName = uigetdir;
...
is3D = get(hObject,'Value');
if is3D == 1
    set(handles.is4D,'value',0)
    handles.is3D4D = '3D';
end
guidata(hObject,handles);
